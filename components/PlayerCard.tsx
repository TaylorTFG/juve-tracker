...
export function PlayerCard({ player }: { player: PlayerRow }) {
  return (
    <Link href={`/players/${player.provider_id}`} className="card block transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="mt-3 text-sm text-black/70">{player.nationality ?? "Nazionalità n/d"}</div>
